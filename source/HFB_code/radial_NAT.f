      subroutine radial_wf(irad_hf_nat)
       ...
       double precision, allocatable, dimension (:) :: rad_wf_p, 
     &rad_wf_n
       ...
       OrbNOn=0.d0
        do i=0,igrid2
         ...
         do j=1,id
          ...
          val_n=0.d0
          do k=1,id
           val_p=val_p+tran_NOp(k,j)
     &                *R_val(levp(k)%nn,levp(k)%l,levp(k)%j2,bos1,radi)
           val_n=val_n+tran_NOn(k,j)
     &                *R_val(levn(k)%nn,levn(k)%l,levn(k)%j2,bos1,radi)
      ...
      do i=0,igrid2
        write(1,'(1000f10.5)')
     &dble(i)*sizebox/dble(igrid2),
     &((OrbNOp(i,k)*dble(i)*sizebox/dble(igrid2))**2.0d0,k=1,id)
      enddo
      close(1)
     ...
     &((OrbNOp(i,k))**2.0d0,k=1,id)
      ...
      do i=0,igrid2
        write(1,'(1000f10.5)')
     &dble(i)*sizebox/dble(igrid2),
     &((OrbNOn(i,k)*dble(i)*sizebox/dble(igrid2))**2.0d0,k=1,id)
      enddo
      close(1)
...
!          rad_wf_p(j)=val_p**2.0d0
!          rad_wf_n(j)=val_n**2.0d0
...
!         write(98,'(1000f10.5)')radi,(rad_wf_n(j),j=1,id)
       ...
       open(47,file='rad_density_'//hfnat//'.out',status='unknown',
     &form='formatted')
        rad_sum=0.d0
        ...
        open(47,file='rad_density_n_'//hfnat//'.out',status='unknown',
     &form='formatted')
          rad_sum=0.d0
          dx=sizebox/dble(igrid2)
          do i=0,igrid2
          ...
          open(47,file='rad_density_p_'//hfnat//'.out',status='unknown',
     &form='formatted')
          rad_sum=0.d0
          dx=sizebox/dble(igrid2)
          do i=0,igrid2
