...
!      call readc2(fnamex,xx,nphon(nff),nphon(1),nphon(1))
      ...
      do ipa=-1,1,2
       do ja=0,jamax
        ...
        if (nb.ne.0) call redrsumx(ilax,ilax+nb,cc,ipozbr,nd)
...
!             do i=1,nbb
       ...
       rog(ila,isi)=rog(ila,isi)+fact*cc(iaaa,i)%am*xx(ibpoz,il1,il)*csixj(isi,jilap,jila,jil1,ja,jb)
       ...
       rog(il1,isi)=rog(il1,isi)+fact*cc(iaaa,i)%am*xx(ibpoz,il,ila)*csixj(isi,jilap,jil1,jila,ja,jb)
...
!            endif
            ...
            if (dabs(rog(ila,isi)).gt.1.d-8) then 
             roc(iroc)%rho=rog(ila,isi)       
             roc(iroc)%ib=ibbb
             roc(iroc)%is=isi
             roc(iroc)%i1=ila
             roc(iroc)%i2=il
...
!     *,phonbs,nphon,fnamern,iaaa)
        ...
        call ropar2(imin,imax,ityp,iph,jamax,jbmax,1,roc,iroc-1,ifile,phonbs,nphon,fnamern,iaaa)
...
!       if (xene.le.xthre) then
       if (jjf.eq.0.and.xene.le.150.0d0) then 
        ii=ii+1
        iphous(i)=1
       endif
      enddo

      write(*,*)' Number of selected phonons 0+',ii
...
!       if (xene.le.xthre) then
       if (jjf.eq.1.and.xene.lt.10.0d0) then
        ii=ii+1
        iphous(i)=1
       endif
      enddo

      write(*,*)' Number of selected phonons 1- ',ii
      ns2=ii
