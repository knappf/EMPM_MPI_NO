...
!     call readc2(fnamex,xx,nphon(nff),nphon(1),nphon(1))
      ...
      call readc2_part(fnamex,xx,ns2,nphon(1),nphon(1)
     *,iphous2,ipozl)
...
!          jb=phonbs(nff,ibbb)%jj
       ...
       rog(ibbb,ila,isi)=rog(ibbb,ila,isi)+fact*
     *cc(iaaa,i)%am*xx(ibbb,il1,il)*csixj(isi,jilap,jila,jil1,ja,jb)

            endif

            if (dabs(csixj(isi,jilap,jil1,jila,ja,jb)).gt.1.0d-10) then

            ifaz=(-1)**(isi+jila+jb+jil1)
            fact=dfloat(ifaz)*(dfloat(2*ja+1))**0.5d0
             
       rog(ibbb,il1,isi)=rog(ibbb,il1,isi)+fact*
...
!************************************************************************
     ...
     *ifile,phonbs,nphon,fnamern,iaaa)
