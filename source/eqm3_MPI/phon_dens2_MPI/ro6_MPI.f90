...
!         isi_mx=ja+jb
      ...
      do il=1,nphon(1) ! lambda'
          ...
          do i=1,nbb
            ...
            rog(0:isimax,ila)=rog(0:isimax,ila)+fact*cc(1,i)%am*xx(ibbb,il1,il)*csixj(0:isimax,jilap,jila,jil1,ja,jb)

            ifaz2=(-1)**(jila+jb+jil1)
            fact2=dfloat(ifaz2)*(dfloat(2*ja+1))**0.5d0
                     
            rog(0:isimax,il1)=rog(0:isimax,il1)+fact2*cc(1,i)%am*xx(ibbb,il,ila)*csixj(0:isimax,jilap,jil1,jila,ja,jb)
          ...
          enddo ! over i
            ...
            do ila=1,nphon(1)
                do isi=0,isimax
                      ...
                      if (dabs(rog(isi,ila)).gt.1.d-8) then 
                          iroc=iroc+1    
      ...
      enddo  !over il
       ...
       if (iroc.ne.0) then 
         call ropar3(iaaa,ibbb,ippmn,ippmx,isimax,roc,iroc,nphon,rpp,r2pp,ndr2pp,ndrh)  
         call ropar3(iaaa,ibbb,ipnmn,ipnmx,isimax,roc,iroc,nphon,rnp,r2np,ndr2np,ndrh)
         call ropar3(iaaa,ibbb,ihpmn,ihpmx,isimax,roc,iroc,nphon,rph,r2ph,ndr2ph,ndrh) 
         call ropar3(iaaa,ibbb,ihnmn,ihnmx,isimax,roc,iroc,nphon,rnh,r2nh,ndr2nh,ndrh)  
       endif
...
!************************************************************************
      subroutine ropar3(iaaa,iba,imin,imax,isimax,rom,irom,nphon,rxy,rh,iirg,ndrh)
      ...
      character(len=6) nlam
            ...
            if (iirg.gt.0) then
                       write(nlam,'(i6.6)')iaaa
                        ...
                        close(73)
            write(733,*)iaaa,iirg,name_new
            do ii=1,iirg
            write(733,*)rh(ii)%ib,rh(ii)%is,rh(ii)%i2,rh(ii)%i1
            write(732,*)rh(ii)%rho
            end do
