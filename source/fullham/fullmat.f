...
******************************************************************************
      ...
      subroutine nondiag1(nff,nbs,ndmpho,phonbs,ipcal,jcal)
     ...
     *vred
      ...
      character*30 fname,fnamev,fnameo
      ...
      nfona=200  ! nff-1 phonon dimension
      nfong=14000  ! nff-1 phonon dimension
      nfons=104    ! 1 phonon dimension
      ndxcc=100000

      allocate (ipoza(nfong),ipozinv(nfona))
      ipoza=0
      ipozinv=0

      write(*,*)'aaaa'

      allocate(vred(nfona,nfong,nfons))
...
c************************************************************************
     ...
     *vred
      ...
      nfona=400  ! nff-1 phonon dimension
      nfong=4000  ! nff-1 phonon dimension
      nfons=4000    ! 1 phonon dimension
      ndxcc=100000
...
!            vint(i)=vint(i)+xcc(ii,kk)%am*            !  ?????? kery je spravne
!     *vred(ipoza(igi),jj,isi) 

            vint(i)=vint(i)+xcc(ii,kk)%am*
     *vred(ipoza(jj),igi,isi)
...
c      write(*,*)'ccc'
      ...
      allocate(xccr(ilamp:ilamp+no,ndimj))
