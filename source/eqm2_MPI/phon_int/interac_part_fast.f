...
!      jp2=lev(ip2)%j
        ...
        do ii=1,nropps
         ...
         do ip2=ipmin,ipmax 
         vint(ip2)=vint(ip2)+0.5d0*dfloat(ifaz)
     ...
     **ronp(ironp(ii))%ro*fpn(isi,i1,i2,ip1,ip2)
        ...
        do ii=1,nrophs
         ...
         do ip2=ipmin,ipmax
          vint(ip2)=vint(ip2)+
     *dfloat(ifaz)*roph(iroph(ii))%ro*fp(isi,i1,i2,ip1,ip2)
         enddo
...
!           xsixj=sixj(2*jib,2*isi,2*jig,jp1,jh,jp2)          
     ...
     *((2*jib+1)*(2*jig+1)*(2*isi+1))**0.5d0
           ...
           vint(ip2)=vint(ip2)-factor*campp*campn*
     *sixj(2*jib,2*isi,2*jig,jp1,jh,jp2)*fpn(jib,ipp,ihp,ih,ip2)
       ...
       do ip2=ipmin,ipmax
       if (dabs(vint(ip2)).gt.xrotrunc) then
               iirg=iirg+1
               if (iirg.gt.ndrho) then
                  write(*,*)' Increase dimension of ndrho in rop!!'
                  stop
