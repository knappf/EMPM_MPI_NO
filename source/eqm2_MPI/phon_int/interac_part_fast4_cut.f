...
!      write(5)ig
      ...
      if (ius(ig).ne.0) then
       ...
       open(5,file='scratch/'//namev//'_'//nlam,status='unknown',
     *form='unformatted')
...
!      ifaz=(-1)**(jib+isi)
     ...
     *ropp(iropp(ii))%ro*fp(isi,i1,i2,ih1,ih2)
        enddo
     ...
     **ronp(ironp(ii))%ro*fpn(isi,i1,i2,ih1,ih2)
        ...
        do ii=1,nrophs
         ...
         isi=roph(iroph(ii))%j
         ifaz=(-1)**(jib+isi)
         vint(isi)=vint(isi)+0.5d0*dfloat(ifaz)*roph(iroph(ii))%ro
...
!      include 'formats_eqm.inc'
      ...
      open(ifile,file='scratch/'//fname//'_'//nlam,status='unknown',
     *form='unformatted')
